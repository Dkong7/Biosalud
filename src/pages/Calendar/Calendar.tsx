...
export const Calendar = () => {
  ...
  // Datos simulados (Idealmente vendrían del CMS)
  const events = [
  ...
  const handleReserve = () => {
    ...
    if (selectedEvent) {
       msg = `Hola Biosalud, me interesa reservar cupo para el evento "${selectedEvent.title}" el día ${dateStr}.`;
    } else {
       msg = `Hola Biosalud, quisiera consultar disponibilidad para visitar la finca el día ${dateStr}.`;
    }

    const url = `https://wa.me/573112024768?text=${encodeURIComponent(msg)}`;
    ...
    <div className="w-full bg-[#f8f9f3] min-h-screen pb-20 animate-fadeIn relative">
      ...
      <div className="container mx-auto px-6 mt-12 max-w-5xl">
        ...
        <div className="flex justify-between items-center mb-8 bg-white p-4 rounded-sm shadow-sm">
           ...
           <h2 className="font-eras text-2xl font-bold text-gray-800 capitalize">
        ...
        <div className="bg-white rounded-sm shadow-lg overflow-hidden">
           ...
           <div className="grid grid-cols-7 border-l border-t border-gray-200">
              {Array.from({ length: firstDay }).map((_, i) => (
                 <div key={`empty-${i}`} className="h-24 md:h-32 border-r border-b border-gray-200 bg-gray-50"></div>
              ))}
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                 ...
                 return (
                    <div 
                      key={day} 
                      onClick={() => handleDateClick(day, evt)}
                      className="h-24 md:h-32 border-r border-b border-gray-200 p-2 relative group hover:bg-bio-verde/5 transition-colors cursor-pointer"
                    >
                       <span className="font-eras font-bold text-gray-400 group-hover:text-bio-verde">{day}</span>
                       
                       {evt && (
                          <div className={`mt-2 p-1 md:p-2 rounded-sm text-[10px] md:text-xs font-bold text-white shadow-sm
      ...
      {selectedDate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
           <div className="bg-white rounded-sm shadow-2xl p-8 max-w-md w-full relative">
              ...
              <button onClick={closeModal} className="absolute top-4 right-4 text-gray-400 hover:text-bio-rojo text-xl">
                 <FontAwesomeIcon icon={faTimes} />
              </button>

              <div className="text-center mb-6">
                 <FontAwesomeIcon icon={faCalendarCheck} className="text-4xl text-bio-verde mb-3" />
                 <h3 className="font-eras text-2xl font-bold text-gray-800">
                    {selectedDate.toLocaleDateString(i18n.language, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                 </h3>
              </div>

              {selectedEvent ? (
                 <div className="bg-gray-50 p-4 rounded-sm border-l-4 border-bio-verde mb-6">
                    <h4 className="font-bold text-bio-verde text-lg">{selectedEvent.title}</h4>
